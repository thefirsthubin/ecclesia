...
const ATTENDANCE_CATEGORY_OPTIONS: { value: GetAttendanceTrendQuery['gatheringCategory'] | 'ALL'; label: string }[] = [
  ...
  { value: 'BACENTA_MEETING', label: 'Bacenta Meeting' },
];
...
function AttendanceSection({ accessToken, groupId, filter, granularityLabel }: { accessToken: string | undefined; groupId: string; filter: GroupTrendFilter; granularityLabel: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[4] }}>
      ...
      <Card padding={6} elevation={1} testId="bacenta-attendance-chart-card">
        <TrendPanel
        ...
        >
          {trendState.status === 'success' &&
            (trendState.data.buckets.every((bucket) => bucket.presentCount === 0) ? (
              <EmptyState icon="trendingUp" title="No attendance recorded" description="No attendance has been recorded for your Bacenta in this window yet." />
            ) : (
              <LineChart data={trendState.data.buckets.map((bucket) => ({ label: bucket.label, value: bucket.presentCount }))} height={200} testId="bacenta-attendance-line-chart" />
            ))}
        </TrendPanel>
      </Card>
...
function GivingSection({ accessToken, groupId, filter, granularityLabel }: { accessToken: string | undefined; groupId: string; filter: GroupTrendFilter; granularityLabel: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[4] }}>
      ...
      <Card padding={6} elevation={1} testId="bacenta-giving-chart-card">
        <TrendPanel
        ...
        >
          {trendState.status === 'success' &&
            (trendState.data.buckets.every((bucket) => bucket.totalAmountMinor === '0') ? (
              <EmptyState icon="trendingUp" title="No giving recorded" description="No giving has been recorded for your Bacenta in this window yet." />
            ) : (
              <LineChart data={trendState.data.buckets.map((bucket) => ({ label: bucket.label, value: Number(bucket.totalAmountMinor) / 100 }))} height={200} testId="bacenta-giving-line-chart" />
            ))}
        </TrendPanel>
      </Card>
...
function MembershipSection({ accessToken, groupId, filter, granularityLabel }: { accessToken: string | undefined; groupId: string; filter: GroupTrendFilter; granularityLabel: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[4] }}>
      ...
      <Card padding={6} elevation={1} testId="bacenta-membership-chart-card">
        <TrendPanel
          title={`${granularityLabel} Membership Growth`}
          description={series === 'members' ? 'Confirmed Members of your Bacenta' : 'Every registered Person on your Bacenta'}
          status={trendState.status}
          onRetry={trendState.refetch}
          ...
          {trendState.status === 'success' &&
            (() => {
              ...
              return points.length === 0 ? (
                <EmptyState icon="trendingUp" title="No membership history yet" description="No membership data is available for your Bacenta in this window yet." />
              ) : (
                <LineChart data={points.map((point) => ({ label: point.label, value: point.value }))} height={200} testId="bacenta-membership-line-chart" />
              );
            })()}
        </TrendPanel>
      </Card>
...
/**
 * `[Milestone D — Portal Experiences, Portal 3: Bacenta Leader]` Insights -
 * "attendance, membership, giving, growth trends... OWN_GROUP scoped."
 * Every control here maps to a real query param on one of the three
 * Milestone C trend endpoints (`useGroupLeaderInsightsData.ts`'s own doc
 * comment covers the RBAC/scope tracing) - `groupId` is always this
 * leader's own `actor.bacentaId`, never user-editable, so every number on
 * this page is genuinely their own Bacenta's, never Branch-wide.
 */
export function BacentaLeaderInsightsView() {
  const theme = useTheme();
  const { state } = useAuth();
    ...
    <PageContainer maxWidth={1280}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[5] }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[3] }}>
          <SectionHeader title="Your Bacenta, over time" />
