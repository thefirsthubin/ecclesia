 ...
 * the two explicit decisions that shaped this: keep Ecclesia's own brand
 * color rather than the reference's pink/lavender palette, and use a top
 * pill nav - `AppShell`'s `navVariant="pill"`, wired from `app.tsx`'s
 * `/dashboard` route only - instead of the sidebar every other page keeps).
 *
 ...
 * route already in `app.tsx`. The KPI/Events/Growth/Leaderboard/Prayer
 * sections read from `useDashboardDemoMetrics` - realistic, disclosed
 * demo data, not a live fetch - since no aggregate endpoint for those
 * exists yet.
 */
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[5], maxWidth: 1280 }}>
      ...
      <div {...fadeIn(40, { display: 'grid', gridTemplateColumns: `repeat(${kpiColumns}, 1fr)`, gap: theme.spacing[4] })}>
        {demoState.status === 'success'
          ? demoState.data.kpis.map((kpi) => <KpiCard key={kpi.id} datum={kpi} testId={`kpi-card-${kpi.id}`} />)
      ...
      <div {...fadeIn(80, heroColumnStyle)}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[4] }}>
          {demoState.status === 'success' && (
            <PerformanceChartCard attendance={demoState.data.growth.attendance} membership={demoState.data.growth.membership} giving={demoState.data.growth.giving} />
          )}
          <div style={pairColumnStyle}>
            ...
            {demoState.status === 'success' && <BacentaLeaderboardCard entries={demoState.data.leaderboard} />}
          </div>
        </div>
