...
/** `[Milestone C.1.2: Outreach Analytics]` `GET
 * /insights/outreach-conversion` - reuses `outreach.contact.read`.
 * Identical shape to `GivingTrendResourceContextGuard` - see that
 * class's own doc comment for the full reasoning. */
@Injectable()
export class OutreachConversionResourceContextGuard extends EcclesiaContextGuardBase {
  constructor(
