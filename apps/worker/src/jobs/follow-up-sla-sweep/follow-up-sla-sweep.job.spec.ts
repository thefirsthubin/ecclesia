...
describe('FollowUpSlaSweepJob', () => {
  function buildJob() {
    const repository = { listBranches: jest.fn(), listOpenTasksWithDueDate: jest.fn() };
    const publisher = { publish: jest.fn() };
    const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    const job = new FollowUpSlaSweepJob(repository as never, publisher as never, logger as never);
