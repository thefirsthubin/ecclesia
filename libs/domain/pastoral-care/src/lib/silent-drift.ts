 ...
 * unchanged.
 */
export function evaluateSilentDrift(input: SilentDriftEvaluationInput): SilentDriftOutcome {
  // Node A
  if (!input.hasActiveBacentaAssignment) {
    return {
      ...
      reason:
        'PRD §15.8 node Z1 / §19.3 Exceptions: no active Bacenta assignment - a BR-PPL-01 data-integrity issue, not a silent-drift evaluation',
    };
  }
