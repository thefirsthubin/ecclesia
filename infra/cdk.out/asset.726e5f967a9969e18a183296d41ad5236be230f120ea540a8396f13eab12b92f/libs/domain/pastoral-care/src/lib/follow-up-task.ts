 ...
 */
export function determineFollowUpTaskTrigger(toStage: string, fromStage?: string): FollowUpTaskTrigger | null {
 ...
 */
export const DEFAULT_FOLLOW_UP_SLA_DAYS: Record<FollowUpTaskTrigger, number> = {
  FIRST_TIME_GUEST: 3,
 ...
 */
export function computeFollowUpTaskDueAt(trigger: FollowUpTaskTrigger, createdAt: Date, slaDaysOverride?: number): Date {
  const slaDays = slaDaysOverride ?? DEFAULT_FOLLOW_UP_SLA_DAYS[trigger];
 ...
 */
export function isFollowUpTaskPastSla(input: FollowUpTaskSlaInput): boolean {
