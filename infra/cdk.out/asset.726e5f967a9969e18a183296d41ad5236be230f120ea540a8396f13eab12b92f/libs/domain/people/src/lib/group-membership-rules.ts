 ...
 */
export function planGroupMembershipChange(
  targetGroupId: string,
  targetGroupType: GroupType,
  activeMemberships: ActiveGroupMembershipRef[],
): GroupMembershipChangePlan {
  const alreadyActiveInTargetGroup = activeMemberships.some((m) => m.groupId === targetGroupId);
  if (alreadyActiveInTargetGroup) {
    throw new Error(
      `Person already holds an active ${targetGroupType} membership in group '${targetGroupId}' - not a valid reassignment or duplicate join`,
    );
  }
